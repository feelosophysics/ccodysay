...
**A2.** 
`GROUP BY`는 테이블의 행들을 특정 컬럼의 값을 기준으로 동일한 그룹(바구니)으로 묶는 역할을 합니다. 이후 지정된 집계 함수들이 각 바구니 내의 데이터들을 대상으로 연산을 수행하여 하나의 요약 결과값을 도출합니다.

#### 💡 실제 예시: 카테고리별 도서 개수 및 평균 가격 조회 (Query 9)
```sql
...
#### ⚙️ 동작 메커니즘
1. **그룹화(Grouping)**: `GROUP BY c.id, c.name`이 실행되면서 카테고리별로 가상의 바구니가 생성되고 데이터가 나뉩니다.
   * **'컴퓨터과학' 바구니**: 
     * `밑바닥부터 시작하는 딥러닝` (가격: 28,000원)
     * `클린 코드` (가격: 33,000원)
     * `Java의 정석` (가격: 32,000원)
   * **'과학/수학' 바구니**: 
     * `코스모스` (가격: 19,500원)
2. **집계 함수 계산(Aggregation)**: 각 바구니 안에서 요약 계산이 수행됩니다.
   * **'컴퓨터과학'**: `COUNT(b.id)`는 도서 개수 **3** / `AVG(b.price)`는 `(28000+33000+32000)/3` = **31,000원**
   * **'과학/수학'**: `COUNT(b.id)`는 도서 개수 **1** / `AVG(b.price)`는 `19500/1` = **19,500원**
3. **결과 출력**:
