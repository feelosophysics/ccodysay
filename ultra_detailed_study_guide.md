...
미션 문서(`b2-1_mission.md`)의 첫 줄은 이렇게 시작합니다:
...
## 1.5 미션 평가 기준 — 무엇을 알아야 통과하는가?

미션 체크리스트(`b2-1_chekclist.md`)를 분석하면, 평가는 4개 항목군으로 나뉩니다:

### 항목 1: "기능이 제대로 동작하는가?" (7개 기준)
- add/list/search/summary/export/import/update/delete가 요구사항대로 동작하는가?
- 프로그램 재실행 후에도 데이터가 유지되는가?
- category 삭제 시 사용 중인 카테고리 처리가 되는가?
- budget set + summary 예산 사용률이 출력되는가?
- import/export가 CSV 스키마 규격대로 동작하는가?
- 오류 시 스택트레이스 없이 힌트가 출력되는가?
- 오류 시 종료 코드가 0이 아닌가?

### 항목 2: "구조를 설명할 수 있는가?" (3개 기준)
- 3개 이상 모듈로 **어떻게** 책임을 나눴는지
- 2개 이상 클래스의 책임 경계를 **어떻게** 정했는지
- update/delete를 **어떻게** 안전하게 처리했는지

### 항목 3: "왜 그렇게 했는지 설명할 수 있는가?" (3개 기준)
- 제너레이터를 **왜** 쓰고, **어떻게** 구현했는지
- 데코레이터를 **왜** 분리하고, **무엇**을 분리했는지
- 타입 힌트의 **이점**을 실제 코드 예로 설명할 수 있는지

### 항목 4: "더 깊이 생각할 수 있는가?" (3개 기준)
- JSONL vs CSV 장단점을 비교하고 **왜** 그것을 택했는지
- 10만 건으로 늘어나면 **어디**가 병목이고 **어떻게** 개선하는지
- 깨진 CSV 행은 **어떻게** 처리하는지
...
# 🏆 10단계 (최종): 평가 대비 — 16가지 기준 완전 정복 (모범 답안)

> **이 단계의 목표**: 지금까지 배운 모든 지식을 총동원하여, 미션 평가 체크리스트(`b2-1_chekclist.md`)의 16개 항목에 대한 "완벽한 면접용/발표용 모범 답안"을 내재화합니다. 이 답안을 소리 내어 읽으며 자신이 직접 짠 코드인 것처럼 설명할 수 있어야 합니다.
